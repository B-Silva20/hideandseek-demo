...
test('all sources produce normalized text with the correct sourceType', () => {
  for (const sourceType of ['text', 'txt'] as const) {
...
test('reject empty, short, symbolic, binary, corrupt and oversized content', () => {
  for (const text of [' \n\t', '案件', '！'.repeat(100), story + '\0', story + '\uFFFD', '案'.repeat(1_000_001)]) {
  ...
  assert.equal(prepareCaseInput('案'.repeat(1_000_000), 'text').text.length, 1_000_000)
})
...
test('TXT reads, extension checks, empty files and file IO failures', async () => {
  ...
  await assert.rejects(readCaseFile(new File([new Uint8Array(MAX_FILE_BYTES + 1)], 'large.txt')), /16 MB/)
